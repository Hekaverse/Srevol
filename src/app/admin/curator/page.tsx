      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-12">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="mt-2 text-warm-white/40">AI scoring engine that identifies the most romantic travel products.</p>
          </div>
          <button
            onClick={runCuration}
            disabled={running}
        ...
        {results && (
          <div className="space-y-4">
            {results.map((r, i) => (
              <div key={r.productId} className="p-6 bg-plum-800/30 rounded-2xl border border-plum-700/30">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-plum-700 flex items-center justify-center">
                      <span className="text-sm font-bold text-rose-gold">#{i + 1}</span>
                    </div>
                    <div>
                      <h3 className="text-lg font-bold text-warm-white">{r.name}</h3>
                      <div className="flex flex-wrap gap-2 mt-2">
                        {r.tags.slice(0, 6).map((tag) => (
                          <span key={tag} className="text-xs px-2 py-1 rounded-full bg-plum-700/50 text-warm-white/50">
                            {tag}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="text-right">
                    <div className="font-serif text-3xl font-bold text-rose-gold">{r.score}</div>
                  ...
                  <ul className="space-y-1">
                    {r.reasons.map((reason, j) => (
                      <li key={j} className="text-sm text-warm-white/50 flex items-center gap-2">
                        <span className="w-1 h-1 rounded-full bg-rose-gold/50" />
                        {reason}
                      </li>
                    ))}
                  </ul>
