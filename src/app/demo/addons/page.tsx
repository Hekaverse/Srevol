      ...
      <main className="pt-32 pb-24">
        <div className="max-w-6xl mx-auto px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="text-xs font-medium text-rose-gold tracking-[0.4em] uppercase">Demo</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-plum-800/50 rounded-2xl border border-plum-700/30 p-6">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 rounded-full bg-plum-600 flex items-center justify-center">
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 rounded-xl border cursor-pointer transition-all ${
                        selected ? "border-rose-gold/40 bg-rose-gold/5" : "border-plum-700/30 hover:border-plum-600"
                      }`}
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${selected ? "border-rose-gold bg-rose-gold" : "border-plum-600"}`}>
                          {selected && <svg className="w-3 h-3 text-plum-900" fill="currentColor" viewBox="0 0 24 24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" /></svg>}
                        </div>
                      </div>
                      {selected && (
                        <button onClick={(e) => { e.stopPropagation(); toggleSelection(setP1Secrets, p1Secrets, addOn.id); }}
                          className={`mt-2 text-xs px-2 py-1 rounded-full transition-colors ${secret ? "bg-rose-gold text-plum-900" : "bg-plum-700 text-warm-white/40 hover:bg-plum-600"}`}>
            ...
            <div className="space-y-6">
              <div className="bg-plum-800/50 rounded-2xl border border-plum-700/30 p-6">
                <h3 className="font-bold text-warm-white mb-4 flex items-center gap-2">
                  <svg className="w-5 h-5 text-blush" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="space-y-4">
                    {schedule.map((day) => (
                      <div key={day.dayOffset}>
                        <h4 className="text-xs font-semibold text-rose-gold uppercase tracking-wider mb-2">Day {day.dayOffset + 1}</h4>
